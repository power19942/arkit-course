...
class ViewController: UIViewController,UICollectionViewDataSource , UICollectionViewDelegate,ARSCNViewDelegate{
    ...
    func registerGestureRecognizer(){
        ...
        let pinchRec = UIPinchGestureRecognizer(target: self, action: #selector(pinch))
        let longPressRec = UILongPressGestureRecognizer(target: self, action: #selector(rotate))
        ...
        self.sceneView.addGestureRecognizer(longPressRec)
    }
    
    @objc func tapped(sender:UITapGestureRecognizer){
        let sceneView = sender.view as! ARSCNView
        let tapLocation = sender.location(in: sceneView)
        // check if you tapped on horizontal plane surface
    ...
    @objc func rotate(sender:UILongPressGestureRecognizer){
        ...
        let holdLocation = sender.location(in: scenview)
        let hitTest = scenview.hitTest(holdLocation)
        if !hitTest.isEmpty{
            let results = hitTest.first!
